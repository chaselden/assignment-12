...
function getFullNames(customersObj){
      var wholeName =[]

  for(var i = 0; i < customersObj.length; i++){
      wholeName.push(customersObj[i].first + ' ' + customersObj[i].last)
 }
 return wholeName
}
...
// pluck(stooges, 'age') should yield the array, [40, 50, 60]
// pluck(stooges, 'hairStyle') should yield the array, ['bowl cut', 'balding', 'buzzing']
...
function pluck(newGuy, propertyStr){

  var pluckedStuff = []


  for(var i=0 ; i < newGuy.length; i++){

      pluckedStuff.push(newGuy[i][propertyStr])
